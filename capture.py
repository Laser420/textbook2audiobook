...
# ── Region selector (used by `new --reselect`) ───────────────────────────────

def select_region() -> dict | None:
    """
    Show a semi-transparent overlay and let the user drag-select a region.
    Returns a region dict in logical pixel coordinates.
    Must be called from the main thread (no other threads running).
    """
    root = tk.Tk()
    ...
    root.geometry(f"{log_w}x{log_h}+0+0")
    root.attributes("-topmost", True)
    root.attributes("-alpha", 0.25)
    root.configure(bg="#000020")

    canvas = tk.Canvas(root, bg="#000020", cursor="crosshair", highlightthickness=0)
    canvas.pack(fill=tk.BOTH, expand=True)
    canvas.focus_set()

    canvas.create_text(
        log_w // 2, 36,
        text="Drag to select the textbook area  —  Esc to cancel",
        fill="white", font=("Helvetica", 17, "bold"),
    )
    dims_label = canvas.create_text(
        log_w // 2, 68, text="", fill="#00ff88", font=("Courier", 14),
    )
    ...
    def on_drag(e: tk.Event) -> None:
        ...
        state["rect"] = canvas.create_rectangle(
            x1, y1, e.x, e.y, outline="#00ff88", width=2,
        )
        canvas.itemconfigure(dims_label, text=f"{abs(e.x - x1)} × {abs(e.y - y1)}")
    ...
    def on_release(e: tk.Event) -> None:
        ...
        if w < 50 or h < 50:
            canvas.itemconfigure(dims_label, text="Too small — try again")
            return
        result.update({
            "left":   int(min(x1, x2)),
            "top":    int(min(y1, y2)),
